...
```sh
curl -fsSL "https://raw.githubusercontent.com/MuhDur/oraclemcp/main/install.sh?$(date +%s)" | bash -s -- --version 0.7.0
```
...
`0.7.0`; change only the version number when installing another release. Later
...
```sh
curl -fsSL "https://raw.githubusercontent.com/MuhDur/oraclemcp/main/install.sh?$(date +%s)" | bash -s -- --dry-run --version 0.7.0
```

From an installed binary, preview or run the same update path:

```sh
oraclemcp --json self-update --dry-run --version 0.7.0
oraclemcp self-update --version 0.7.0 --no-service
```
...
powershell -ExecutionPolicy Bypass -File .\install.ps1 -DryRun -Version 0.7.0
...
```powershell
powershell -ExecutionPolicy Bypass -File .\install.ps1 -Update -Version 0.7.0 -NoService
```
...
```sh
bash install.sh --offline ./oraclemcp-x86_64-unknown-linux-musl.tar.gz --version 0.7.0
```

```powershell
powershell -ExecutionPolicy Bypass -File .\install.ps1 `
  -Offline .\oraclemcp-x86_64-pc-windows-msvc.zip -Version 0.7.0
```
...
docker run -i --rm ghcr.io/muhdur/oraclemcp:0.7.0
```
...
0.6.0 driver itself is stable-clean. The repository's `rust-toolchain.toml`
...
docker run -i --rm \
  ...
  ghcr.io/muhdur/oraclemcp:0.7.0          # MCP over stdio, against the configured profile

docker run -i --rm ghcr.io/muhdur/oraclemcp:0.7.0   # tool surface only (no DB)
```
...
```sh
docker run -i --rm ghcr.io/muhdur/oraclemcp:0.7.0-plsql-intelligence --json info
docker run -i --rm ghcr.io/muhdur/oraclemcp:0.7.0-plsql-intelligence capabilities
```
...
validation, but the pinned `oracledb` 0.6.0 thin adapter **fails closed** on an
...
the `oracledb` 0.6.0 driver is stable-clean. The pin is **build-time only**: the
