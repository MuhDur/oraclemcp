...
```sh
curl -fsSL "https://raw.githubusercontent.com/MuhDur/oraclemcp/main/install.sh?$(date +%s)" | bash -s -- --version 0.6.2
```
...
`0.6.2`; change only the version number when installing another release. Later
...
```sh
curl -fsSL "https://raw.githubusercontent.com/MuhDur/oraclemcp/main/install.sh?$(date +%s)" | bash -s -- --dry-run --version 0.6.2
```

From an installed binary, preview or run the same update path:

```sh
oraclemcp --json self-update --dry-run --version 0.6.2
oraclemcp self-update --version 0.6.2 --no-service
```
...
powershell -ExecutionPolicy Bypass -File .\install.ps1 -DryRun -Version 0.6.2
powershell -ExecutionPolicy Bypass -File .\install.ps1 -Version 0.6.2
```
...
```powershell
powershell -ExecutionPolicy Bypass -File .\install.ps1 -Update -Version 0.6.2 -NoService
```
...
```sh
bash install.sh --offline ./oraclemcp-x86_64-unknown-linux-musl.tar.gz --version 0.6.2
```

```powershell
powershell -ExecutionPolicy Bypass -File .\install.ps1 `
  -Offline .\oraclemcp-x86_64-pc-windows-msvc.zip -Version 0.6.2
```
...
docker run -i --rm ghcr.io/muhdur/oraclemcp:0.6.2
```
...
npm view oraclemcp@0.6.2 version
```
...
docker run -i --rm \
  ...
  ghcr.io/muhdur/oraclemcp:0.6.2          # MCP over stdio, against the configured profile

docker run -i --rm ghcr.io/muhdur/oraclemcp:0.6.2   # tool surface only (no DB)
```
...
```sh
docker run -i --rm ghcr.io/muhdur/oraclemcp:0.6.2-plsql-intelligence --json info
docker run -i --rm ghcr.io/muhdur/oraclemcp:0.6.2-plsql-intelligence capabilities
```
