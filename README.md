...
```sh
curl -fsSL "https://raw.githubusercontent.com/MuhDur/oraclemcp/main/install.sh?$(date +%s)" | bash -s -- --version 0.6.6
```
...
`0.6.6`; change only the version number when installing another release. Later
...
```sh
curl -fsSL "https://raw.githubusercontent.com/MuhDur/oraclemcp/main/install.sh?$(date +%s)" | bash -s -- --dry-run --version 0.6.6
```
...
```sh
bash install.sh --offline ./oraclemcp-x86_64-unknown-linux-musl.tar.gz --version 0.6.6
```
...
docker run -i --rm ghcr.io/muhdur/oraclemcp:0.6.6
```
...
docker run -i --rm \
  ...
  ghcr.io/muhdur/oraclemcp:0.6.6          # MCP over stdio, against the configured profile

docker run -i --rm ghcr.io/muhdur/oraclemcp:0.6.6   # tool surface only (no DB)
```
...
```sh
docker run -i --rm ghcr.io/muhdur/oraclemcp:0.6.6-plsql-intelligence --json info
docker run -i --rm ghcr.io/muhdur/oraclemcp:0.6.6-plsql-intelligence capabilities
```
