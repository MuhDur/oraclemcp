...
| `tests/fixtures/installer_semver_vectors.tsv` | Hand-authored SemVer normalization and ordering vectors shared by the Bash and PowerShell installers | Hand-edit alongside `scripts/installer_lint_and_offline_smoke.sh`; consumed by its POSIX and PowerShell parity checks, with no generator |
...
| `tests/ci_heartbeat/fuzz-setup-job-failure-2026-09-23.runs.json` | Recorded GitHub API response (trimmed to the fields `scripts/ci_heartbeat.sh` reads) for the real scheduled Fuzz Campaign run 35839299781 that failed at "Set up job" (bead .2.1); public repo metadata only | `gh api repos/MuhDur/oraclemcp/actions/runs/35839299781 --jq '{workflow_runs: [{id, event, status, conclusion, html_url, head_sha, updated_at}]}'`; replayed by `scripts/test_ci_heartbeat.sh` |
| `tests/ci_heartbeat/fuzz-setup-job-failure-2026-09-23.jobs.json` | Recorded GitHub API response (trimmed) for the 8 jobs of the same run, each failing only at "Set up job" | `gh api 'repos/MuhDur/oraclemcp/actions/runs/35839299781/jobs?filter=latest&per_page=100' --jq '{jobs: [.jobs[] \| {name, status, conclusion, html_url, completed_at, steps: [.steps[] \| {name, status, conclusion}]}]}'`, jobs sorted by name; replayed by `scripts/test_ci_heartbeat.sh` |
| `tests/fixtures/release/oraclemcp-verifier.package-list` | Expected `cargo package --list` file set of the published `oraclemcp-verifier` crate (.4.1); compared by `tests/release_contract_test.sh` so an unintended file never ships | `cargo package --list --allow-dirty -p oraclemcp-verifier`, reviewed by hand before committing |
