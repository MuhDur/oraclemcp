...
| Rollback runbook dry-run | `scripts/e2e/release_rollback_dry_run.sh` | current git checkout plus explicit synthetic broken/previous-good versions | `bash scripts/e2e/release_rollback_dry_run.sh --log --dry-run --broken-version 9.9.9 --previous-good 9.9.8` |
...
| Query cost-gate live matrix | `scripts/e2e/cost_gate.sh` | Three throwaway lab lanes (gvenzl XE 18 / XE 21 / FREE 23ai) with `ORACLEMCP_LIVE_XE=1` and `ORACLE_MATRIX_*` lane credentials | `bash scripts/e2e/cost_gate.sh --log` |
| Time-diff + observed-SCN replay live matrix | `scripts/e2e/time_diff.sh`, `scripts/e2e/time_diff_session.py` | Three throwaway lab lanes (gvenzl XE 18 / XE 21 / FREE 23ai) with `ORACLEMCP_LIVE_XE=1` and `ORACLE_MATRIX_*` lane credentials; evidence and audit chains stay under ignored `target/e2e/` | `bash scripts/e2e/time_diff.sh --log` |
| Served SQL-policy proof | `scripts/e2e/sql_policy.sh`, `scripts/e2e/sql_policy_session.py` | A disposable user and table in the existing local FREE 23ai lab; the harness writes only synthetic rule IDs and digests under ignored `target/e2e/`, and drops the fixture and user after the run | `bash scripts/e2e/sql_policy.sh --log` |
| Served refusal-corpus MCP proof | `scripts/e2e/refusal_corpus.sh`, `scripts/e2e/refusal_corpus_session.py` | Isolated zero-profile stdio server plus only synthetic identifiers, bind name/value, and literal secret; generated state, diagnostics, and evidence remain under ignored `target/e2e/` | `bash scripts/e2e/refusal_corpus.sh --log` |
| Served-console affordance proof | `scripts/e2e/served_console.sh`, `web/src/app/served-console.e2e.test.ts` | The real `oraclemcp serve` binary over loopback Streamable HTTP, paired the way the browser pairs; honesty negatives need no database; the positive path (real proof-carrying record, `observed_scn`, federated fleet map) is offered a throwaway lab DB via `OMCP_LIVE_DSN`/`OMCP_LIVE_USER`/`OMCP_LIVE_CRED` (e.g. gvenzl FREE 23ai) | `OMCP_LIVE_DSN=127.0.0.1:1521/freepdb1 OMCP_LIVE_USER=… OMCP_LIVE_CRED=… bash scripts/e2e/served_console.sh --log` |
| Served governed-egress MCP proof | `scripts/e2e/served_egress.sh`, `scripts/e2e/served_egress_session.py` | FREE 23ai local lab and only run-unique synthetic literals selected from `DUAL`; the real stdio response line is scanned before parsing, audit-chain verification re-derives the response certificate, and generated config/state/evidence remain under ignored `target/e2e/` | `bash scripts/e2e/served_egress.sh --log` |
...
| D3.2 local pre-tag release gate | `scripts/local_release_gate.sh`, `scripts/local_release_gate_check.sh`, `crates/oraclemcp-core/tests/oci_tcps_e2e.rs`, `docs/operator/real-adb-tcps-signoff.md` | Synthetic loopback TCPS terminator and synthetic wallet/cert material only (`CN=oracle-test.invalid`); optional committed proof is sanitized JSON under `tests/artifacts/local_gate/` | `bash scripts/local_release_gate.sh --log --commit-proof` |
| C5 real ADB TCPS + OCI IAM signoff harness | `scripts/e2e/real_adb_tcps_signoff.sh`, `docs/operator/real-adb-tcps-signoff.md` | Operator-supplied runtime ADB wallet and prefetched OCI IAM database token; generated profiles and raw evidence remain under ignored `target/e2e/` and are never committed | `bash scripts/e2e/real_adb_tcps_signoff.sh --log` |
