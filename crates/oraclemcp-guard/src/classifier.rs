...
use crate::classifier_grammar::normalize_vector_embedding_for_parser;
...
mod tests {
    ...
    #[test]
    fn raw_name_from_idents_does_not_parse_dblink_shape_from_quoted_identifiers() {
        let name = raw_name_from_idents(
            &[Ident::with_quote('"', "HR@PROD")],
            SyntacticRole::FromFactor,
        )
        .expect("quoted identifiers remain exact relation members");
        assert_eq!(name.parts.len(), 1);
        assert_eq!(name.parts[0].text, "HR@PROD");
        assert_eq!(name.parts[0].quoting, QuoteSemantics::Quoted);
        assert!(name.db_link.is_none());
    }
    ...
    #[test]
    fn vector_embedding_parser_normalization_rejects_lookalike_spelling() {
        let builtin = "SELECT VECTOR_EMBEDDING(LOCAL_ONNX_MODEL USING :1) FROM dual";
        assert_eq!(
            normalize_vector_embedding_for_parser(builtin),
            "SELECT VECTOR_EMBEDDING(LOCAL_ONNX_MODEL, :1) FROM dual"
        );

        for sql in [
            "SELECT app.VECTOR_EMBEDDING(LOCAL_ONNX_MODEL USING :1) FROM dual",
            "SELECT app /* qualifier */ . /* qualifier */ VECTOR_EMBEDDING(LOCAL_ONNX_MODEL USING :1) FROM dual",
            "SELECT \"VECTOR_EMBEDDING\"(LOCAL_ONNX_MODEL USING :1) FROM dual",
            "SELECT 'VECTOR_EMBEDDING(LOCAL_ONNX_MODEL USING :1)' FROM dual",
            "SELECT VECTOR_EMBEDDING /* comment */ (LOCAL_ONNX_MODEL USING :1) FROM dual",
        ] {
            assert_eq!(
                normalize_vector_embedding_for_parser(sql),
                sql,
                "only the bare built-in's exact grammar may use the parser bridge: {sql}"
            );
        }
    }
