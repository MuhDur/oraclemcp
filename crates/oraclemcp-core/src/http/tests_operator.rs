...
fn change_proposal_apply_preserves_the_exact_lane_generation() {
    ...
    assert_eq!(
        exact["data"]["results"][0]["action_response"]["data"]["idempotency"]["lane_generation"],
        serde_json::json!(7),
...
// Proposal-listing and source-history route coverage stays in a focused sibling
// test document so this operator route-contract suite remains navigable.
include!("tests_operator_proposals.rs");
