...
const CI_LANE_RETRY_INTERVAL: Duration = Duration::from_secs(5 * 60);
const CI_LANE_STALE_AFTER: Duration = Duration::from_secs(60 * 60);
const CI_LANE_REQUEST_TIMEOUT: Duration = Duration::from_secs(5);
const CI_LANE_MAX_RESPONSE_BYTES: usize = 2 * 1024 * 1024;

static CI_LANE_CATALOG: OnceLock<Result<Vec<CiLaneCatalogEntry>, String>> = OnceLock::new();
static CI_LANE_CACHE: OnceLock<Mutex<CiLaneCacheState>> = OnceLock::new();
...
#[derive(Clone, Debug)]
struct CiLaneSnapshot {
    refreshed_at_unix: u64,
    refreshed_at: Instant,
    lanes: Vec<CiLaneHealth>,
    errors: Vec<String>,
}

#[derive(Debug, Default)]
struct CiLaneCacheState {
...
impl OperatorIdempotencyLedger {
    ...
    pub(super) fn complete(
        ...
        if let Some(entry) = entries
            .get_mut(&lease.storage_key)
            .filter(|entry| entry.created_at == lease.created_at)
        {
...
impl Drop for OperatorIdempotencyLease {
    fn drop(&mut self) {
        ...
        let abandoned = entries
            ...
            .is_some_and(|entry| entry.created_at == self.created_at && entry.response.is_none());
        if abandoned {
...
pub(super) fn handle_operator_api_route(
    ...
    match route {
        ...
        OperatorRouteKind::CiLanes => {
            operator_json_response(200, &request.path, operator_ci_lane_health_data())
        }
...
fn operator_ci_lane_health_data() -> Value {
    ...
    let cache = CI_LANE_CACHE.get_or_init(|| Mutex::new(CiLaneCacheState::default()));
    let now = Instant::now();
    let should_spawn = {
        let mut state = cache.lock();
        let refresh_due = match &state.snapshot {
            Some(snapshot) => now.duration_since(snapshot.refreshed_at) >= CI_LANE_REFRESH_INTERVAL,
            None => state
                .last_attempt_at
                .is_none_or(|attempt| now.duration_since(attempt) >= CI_LANE_RETRY_INTERVAL),
        };
        if refresh_due && !state.refreshing {
            state.refreshing = true;
            state.last_attempt_at = Some(now);
            state.last_attempt_unix = Some(unix_now());
            true
        } else {
            false
        }
    ...
    if should_spawn {
        spawn_ci_lane_refresh();
    }

    let state = cache.lock();
    render_ci_lane_health_data(catalog, &state)
}

fn spawn_ci_lane_refresh() {
    let spawn = std::thread::Builder::new()
        .name("oraclemcp-ci-lane-health".to_owned())
        .spawn(|| {
            let result = catch_unwind(AssertUnwindSafe(fetch_ci_lane_snapshot_blocking))
                .map_err(|_| "CI lane refresh panicked".to_owned())
    ...
    if let Err(error) = spawn {
        ...
        let mut state = cache.lock();
        state.refreshing = false;
        state.last_error = Some(format!("could not start CI lane refresh worker: {error}"));
    }
}

fn fetch_ci_lane_snapshot_blocking() -> Result<CiLaneSnapshot, String> {
    let catalog = ci_lane_catalog().map_err(str::to_owned)?.to_vec();
    let reactor = asupersync::runtime::reactor::create_reactor()
        .map_err(|error| format!("CI lane refresh reactor: {error}"))?;
    let runtime = RuntimeBuilder::current_thread()
        .with_reactor(reactor)
        .build()
        .map_err(|error| format!("CI lane refresh runtime: {error}"))?;
    runtime.block_on(async move {
        let cx =
            Cx::current().ok_or_else(|| "CI lane refresh has no runtime context".to_owned())?;
        let client = HttpClient::builder()
            ...
            .user_agent(format!(
                "oraclemcp/{} lane-health",
                env!("CARGO_PKG_VERSION")
            ))
            .default_header("Accept", "application/vnd.github+json")
            .default_header("X-GitHub-Api-Version", "2022-11-28")
            .build();
        Ok(fetch_ci_lane_snapshot(&cx, &client, &catalog).await)
    })
}

async fn fetch_ci_lane_snapshot(
    cx: &Cx,
    client: &HttpClient,
    ...
    for ((workflow_file, event), lanes) in groups {
        match fetch_ci_lane_group(cx, client, &workflow_file, &event, &lanes).await {
            Ok(health) => {
                for lane in health {
...
async fn fetch_ci_lane_group(
    ...
    let runs_url = format!(
        "https://api.github.com/repos/{CI_LANE_GITHUB_REPO}/actions/workflows/{workflow_file}/runs?event={event}&status=completed&per_page={CI_LANE_STREAK_WINDOW}{branch}"
    );
    ...
    for run in runs.workflow_runs {
        let jobs_url = format!(
            "https://api.github.com/repos/{CI_LANE_GITHUB_REPO}/actions/runs/{}/jobs?filter=latest&per_page=100",
            run.id
        );
...
fn render_ci_lane_health_data(catalog: &[CiLaneCatalogEntry], state: &CiLaneCacheState) -> Value {
    let now = Instant::now();
    let (refreshed_at, age_seconds, snapshot_stale, snapshot_errors, health) = match &state.snapshot
    {
        Some(snapshot) => {
            let age = now.duration_since(snapshot.refreshed_at);
            (
                Some(format!("unix:{}", snapshot.refreshed_at_unix)),
                Some(age.as_secs()),
                age >= CI_LANE_STALE_AFTER || state.last_error.is_some(),
        ...
        None => (
            ...
            catalog
                .iter()
                .cloned()
                .map(|lane| unknown_ci_lane(lane, "lane evidence has not refreshed yet"))
                .collect(),
        ),
    };
    let lanes = health
        .iter()
        .map(|lane| ci_lane_health_json(lane, snapshot_stale))
        .collect::<Vec<_>>();
    let success = lanes
        .iter()
    ...
    let freshness = if state.snapshot.is_none() {
        if state.refreshing {
            "refreshing"
        } else {
            "unavailable"
    ...
    let source = if state.snapshot.is_none() {
        "unavailable"
    } else if errors.is_empty() {
        "github_actions"
    } else {
        "github_actions_partial"
    };
    json!({
        "source": source,
        "catalog_schema": CI_LANE_TAXONOMY_SCHEMA,
        ...
        "repo": CI_LANE_GITHUB_REPO,
        "refresh_state": if state.refreshing { "refreshing" } else if state.last_error.is_some() { "failed" } else { "ready" },
        "freshness": freshness,
        "refreshed_at": refreshed_at,
        "last_attempt_at": state.last_attempt_unix.map(|value| format!("unix:{value}")),
