...
pub enum AuditOutcome {
    ...
    RolledBack,
    /// Executed and left **pending inside an open reversible workspace** (Arc I):
    /// uncommitted, undoable to a checkpoint, and — until the workspace is
    /// closed — unable to reach a commit. Neither `Succeeded` (nothing is
    /// durable) nor `RolledBack` (nothing has been undone yet) would be true.
    HeldUncommitted,
...
pub struct AuditRecord {
    ...
    /// System change number for the committed snapshot observed by an audited
    /// read. Absent for non-read records and historical records that predate
    /// forensic replay support.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub observed_scn: Option<u64>,
    /// Domain-separated hash of the redacted verdict-certificate core. Present
    /// only when the audit append received a certificate; it is covered by the
    /// schema-v10 chain hash and HMAC, never by an untrusted response field.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub verdict_certificate_core_hash: Option<String>,
    /// The tool invoked.
    pub tool: String,
...
impl std::fmt::Debug for AuditRecord {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("AuditRecord")
            ...
            .field("observed_scn", &self.observed_scn)
            .field(
                "verdict_certificate_core_hash",
                &self.verdict_certificate_core_hash,
            )
...
impl AuditRecord {
    ...
    /// Build a signed current-schema record with optional attempt/terminal
    /// correlation.
    #[must_use]
    pub fn chained_signed_correlated(
        ...
        Self::chained_signed_correlated_with_observed_scn(
            draft,
            seq,
            prev_hash,
            timestamp,
    ...
    /// Build a signed current-schema record with optional correlation and an
    /// observed read snapshot SCN. The SCN is hash-covered and signed, so a
    /// replay target cannot be changed without breaking chain verification.
    #[must_use]
    pub fn chained_signed_correlated_with_observed_scn(
        draft: &AuditEntryDraft,
        seq: u64,
        prev_hash: &str,
        timestamp: String,
        ...
        Self::chained_signed_correlated_with_observed_scn_and_certificate_core_hash(
            draft,
            seq,
            prev_hash,
            timestamp,
    ...
    /// Build a signed current-schema record with optional correlation, an
    /// observed read snapshot SCN, and a redacted certificate-core hash. The
    /// hash is an opaque correlation value; the certificate itself never enters
    /// the audit record or its debug projection.
    #[must_use]
    #[allow(clippy::too_many_arguments)]
    pub fn chained_signed_correlated_with_observed_scn_and_certificate_core_hash(
        draft: &AuditEntryDraft,
        seq: u64,
        prev_hash: &str,
        timestamp: String,
    ...
    /// Build an unsigned current-schema record with optional attempt/terminal
    /// correlation.
    #[must_use]
    pub fn chained_unsigned_correlated(
        ...
        Self::chained_unsigned_correlated_with_observed_scn(
            draft,
            seq,
            prev_hash,
            timestamp,
    ...
    /// Build an unsigned current-schema record with optional correlation and
    /// an observed read snapshot SCN.
    #[must_use]
    pub fn chained_unsigned_correlated_with_observed_scn(
        draft: &AuditEntryDraft,
        seq: u64,
        prev_hash: &str,
        timestamp: String,
    ...
    /// Build an unsigned current-schema record with optional correlation, an
    /// observed read snapshot SCN, and a redacted certificate-core hash.
    #[must_use]
    pub fn chained_unsigned_correlated_with_observed_scn_and_certificate_core_hash(
        draft: &AuditEntryDraft,
        seq: u64,
        prev_hash: &str,
        timestamp: String,
        ...
        let entry_hash = compute_entry_hash_v10(
            ...
            observed_scn,
            verdict_certificate_core_hash.as_deref(),
            &draft.tool,
            &sql_sha256,
    ...
    pub fn hash_is_valid(&self) -> bool {
        ...
        } else if self.schema_version == AUDIT_SCHEMA_V9 {
            compute_entry_hash_v9(
                self.seq,
                &self.timestamp,
                &self.agent_identity,
                &self.subject,
...
fn canonical_push_observed_scn(out: &mut Vec<u8>, observed_scn: Option<u64>) {
    match observed_scn {
        Some(scn) => {
            out.push(1);
            out.extend_from_slice(&scn.to_be_bytes());
        }
        None => out.push(0),
    }
}

fn canonical_push_verdict_certificate_core_hash(out: &mut Vec<u8>, core_hash: Option<&str>) {
    match core_hash {
        Some(hash) => {
            out.push(1);
            canonical_push_str(out, hash);
        }
        None => out.push(0),
    }
}
...
const fn canonical_outcome_tag(outcome: AuditOutcome) -> u8 {
    match outcome {
        ...
        // Additive: a new tag never changes an existing one, so every record
        // hashed before Arc I still hashes identically and the chain verifies.
...
/// Deterministically hash a v9 entry, extending v8 with the optional SCN of
/// the committed snapshot observed by an audited read.
#[allow(clippy::too_many_arguments)]
fn compute_entry_hash_v9(
    seq: u64,
    timestamp: &str,
    agent_identity: &str,
    subject: &AuditSubject,
...
/// Deterministically hash a v10 entry, extending v9 with the optional
/// domain-separated verdict-certificate core hash.
#[allow(clippy::too_many_arguments)]
fn compute_entry_hash_v10(
    seq: u64,
    timestamp: &str,
    agent_identity: &str,
    subject: &AuditSubject,
...
mod tests {
    ...
    fn signed_record_for_schema_with_draft(
        ...
        let entry_hash = match schema_version {
            ...
            AUDIT_SCHEMA_V9 => compute_entry_hash_v9(
                seq,
                &timestamp,
                &agent_identity,
                &d.subject,
    ...
    #[test]
    fn observed_scn_is_serialized_and_hash_covered() {
        let mut record = AuditRecord::chained_signed_correlated_with_observed_scn(
            &draft(),
            1,
            GENESIS_HASH,
            "2026-07-13T00:00:00Z".to_owned(),
        ...
        assert!(encoded.contains("\"observed_scn\":42000001"));

        record.observed_scn = Some(42_000_002);
        assert!(
            !record.hash_is_valid(),
            "changing a replay target must break the tamper-evident chain"
        );
    }

    #[test]
    fn verdict_certificate_core_hash_is_serialized_and_hash_covered() {
        let certificate_core_hash = sha256_hex(b"verdict certificate core");
        let mut record =
            AuditRecord::chained_signed_correlated_with_observed_scn_and_certificate_core_hash(
                ...
                None,
                Some(42_000_001),
                Some(certificate_core_hash.clone()),
            );
        ...
        assert!(record.signature_is_valid(&key()));
        let encoded = serde_json::to_string(&record).expect("record serializes");
        assert!(encoded.contains("\"verdict_certificate_core_hash\""));

        record.verdict_certificate_core_hash = Some(sha256_hex(b"forged core"));
        assert!(
            !record.hash_is_valid(),
            "changing a certificate core hash must break the tamper-evident chain"
        );
    }
    ...
    fn recompute_from_genesis_without_key_is_detected_by_mac() {
        ...
        forged.entry_hash = compute_entry_hash_v10(
            ...
            forged.observed_scn,
            forged.verdict_certificate_core_hash.as_deref(),
            &forged.tool,
            &forged.sql_sha256,
