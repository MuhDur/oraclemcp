...
fn install_manual_preserves_detailed_contract_and_readme_stays_install_first() {
    ...
    // Third-party package-manager and npm channels are retired (plan
    // R11/R17/R32): no install surface may offer them again.
    for retired in [
        "brew install",
        "brew info",
        "winget install",
        ...
        for (surface, text) in [
            ...
            assert!(
                ...
                "{surface} must not offer the retired channel command {retired}"
            );
        }
    }
}
...
#[test]
fn binstall_metadata_valid_and_no_distribution_manifests_ship() {
    let root = repo_root();
    let manifest =
        fs::read_to_string(root.join("crates/oraclemcp/Cargo.toml")).expect("read Cargo.toml");
    ...
    // Package-manager manifests are retired (plan R17): no template, renderer or
    // release step may come back, and the release uploads no manifests.
    for retired in [
        "packaging/homebrew",
        "packaging/winget",
        "scripts/render_distribution_manifests.sh",
    ] {
        assert!(
            !root.join(retired).exists(),
            "retired distribution-manifest path must not exist: {retired}"
        );
    }
    let workflow =
    ...
    for forbidden in [
        ...
        assert!(
            ...
            "release.yml must not render or upload package-manager manifests: {forbidden}"
        );
    }
}
