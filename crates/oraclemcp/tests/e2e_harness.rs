...
#[test]
fn release_upload_is_sequentially_blocked_by_payload_verification() {
    let workflow = std::fs::read_to_string(repo_root().join(".github/workflows/release.yml"))
        .expect("read release workflow");
    let verify = workflow
        .find("- name: Verify release payloads and proofs before upload")
        .expect("release workflow has payload verification step");
    let upload = workflow
    ...
    assert!(
        workflow[verify..upload]
            .contains("run: bash scripts/verify_release_artifacts.sh artifacts"),
        "release verification must execute the fail-closed checksum/signature gate directly"
    );
    assert!(
        !workflow[verify..upload].contains("continue-on-error"),
        "release verification failure must prevent the upload step"
    );
    // The retired distribution-manifest renderer (plan R17) is gone from the
    // pipeline; checksum binding lives in verify_release_artifacts.sh.
    assert!(
        !workflow.contains("render_distribution_manifests")
            && !workflow.contains("distribution-manifests"),
        "release workflow must not render or upload package-manager manifests"
    );
}
...
fn oci_adb_terraform_dry_run_wires_explicit_teardown() {
    ...
    for expected in [
        ...
        "OCI canonicalizes that value differently across",
        "SELECT USER FROM DUAL",
    ] {
        assert!(
