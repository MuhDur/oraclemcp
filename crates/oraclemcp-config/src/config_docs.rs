...
static DOCS: &[ConfigFieldDoc] = &[
    ...
    doc!("profiles.explain_plan_table", "string(OWNER.TABLE)", "none", true, false, "1", "Optional operator-owned global temporary table for EXPLAIN PLAN; its Oracle object identity is pinned on first verification."),
    ...
    doc!("profiles.require_security_feature_evidence", "bool", "false", true, false, "2", "Refuse reads when OLS/RAS/Data Redaction evidence is unreadable (default admits them with keyed security_feature_evidence: unavailable observations + audit record; a proven policy always refuses; protected profiles imply strict mode)."),
    doc!("profiles.require_hard_parse_evidence", "bool", "false", true, false, "2", "Refuse EXPLAIN and decisive query-cost admission when installed hard-parse callback or PLAN_TABLE evidence is unreadable; default emits a verification observation and separate audit record, and doctor warns only when an online probe finds unreadable evidence."),
    doc!("profiles.require_query_cost_estimate", "bool", "false", true, false, "2", "Refuse a read when requested optimizer cost cannot be measured; independent of require_hard_parse_evidence. Default admits with a cost_unavailable observation and readable audit record."),
    doc!("profiles.diagnostics_pack_licensed", "bool", "false", true, false, "1", "Operator attestation that this Oracle target is licensed for the Diagnostics Pack; AWR also requires the Oracle activation setting and is never enabled from that setting alone."),
...
mod tests {
    ...
    fn maximal_profile() -> ConnectionProfile {
        ConnectionProfile {
            ...
            explain_plan_table: Some("ORACLEMCP.PLAN_TABLE".to_owned()),
            ...
            diagnostics_pack_licensed: Some(true),
