...
| HTTP endpoint | Streamable HTTP is mounted at `/mcp` behind an explicit router; request targets retain parsed query pairs. JSON response and stateful/session behavior are configurable. In stateful mode initialize issues `mcp-session-id`, subsequent POST/GET/DELETE requests must present a known id, POST responses are buffered for GET replay by `cursor` / `Last-Event-ID`, and a cursor older than the retained per-session ring returns typed `410 stream_cursor_expired` instead of silently replaying a truncated suffix. DELETE invalidates the session, clears its replay buffer, and invokes the stateful lane lifecycle hook so the bound lane is closed. Listener shutdown stops accepting, drains workers, then closes all stateful lanes. Stateless DELETE returns 405. Product binaries can serve the embedded operator dashboard outside the API prefix; `/operator/v1` is reserved as a typed JSON API prefix and never falls through to an HTML history fallback. | `crates/oraclemcp-core/src/http.rs`, `crates/oraclemcp-core/src/lane.rs`, `crates/oraclemcp/src/main.rs` |
| HTTP stateful capacity | In served stateful mode, new lane allocation is admitted before the lane thread/factory can open a physical Oracle connection. N4 upper-bound defaults are 8 stateful lanes per principal bucket and 64 total host slots, with 1 operator and 1 doctor/readiness slot reserved outside regular agent admission. Exhaustion returns `AT_CAPACITY` with `retry_after_ms`, HTTP 429 / `Retry-After`, and a redacted capacity snapshot. | `crates/oraclemcp-core/src/admission.rs`, `crates/oraclemcp-core/src/lane.rs`, `crates/oraclemcp-core/src/http.rs`, `crates/oraclemcp/src/main.rs` |
