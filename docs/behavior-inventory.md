...
| Current release line | All `oraclemcp-*` package versions and `server.json` are aligned on the workspace release, while the independently-versioned thin driver/protocol pins are exact at 0.9.0. | `Cargo.toml`, crate `Cargo.toml` files, `server.json`, `Cargo.lock` |
...
| --- | --- | --- |
| `oraclemcp serve` | Serves stdio by default; `--listen` enables HTTP; `--allow-no-auth` disables stdio's init-token requirement only when no token is configured and permits an unauthenticated HTTP listener for intentional local development; non-loopback HTTP still needs explicit remote opt-in. OAuth/Host/Origin/stateful/JSON transport config can come from config or CLI; `--stdio-token` may resolve from `$ORACLEMCP_STDIO_TOKEN`; `--profile` selects active profile. | `crates/oraclemcp/src/main.rs`, `crates/oraclemcp-config/src/lib.rs`, `README.md` |
...
| `oraclemcp setup` | Generates local profile/tool templates and references `ORACLEMCP_STDIO_TOKEN`; `setup --write` applies the minimal bootable starter profiles file through SCFG config-ops and does not echo the raw draft. The full annotated profile reference remains `oraclemcp.example.toml`. Must not print real secrets. | `crates/oraclemcp/src/main.rs` |
| `oraclemcp setup --discover [--discover-tns] [--yes] [--dry-run] [--json]` | Scans `tnsnames.ora` and synthesizes one READ_ONLY profile per net-service. Consent-gated: a non-TTY run without `--discover-tns`/`--yes` refuses with exit 2 and scans nothing (distinct scan- and write-consent). Writes through config-ops with a timestamped backup and verify-before-mutate; add-only merge preserves existing profiles; zero-found falls back to the minimal starter. `max_level`/`default_level` set explicitly to READ_ONLY; `credential_ref` is an `env:` placeholder — no secret is read or written. `--json` emits a names-only report (`net_services`, `profiles`, `env_vars`, `write_mode`, `backup_path`, `target_path`); `--dry-run` previews and writes nothing. | `crates/oraclemcp/src/discover.rs`, `docs/tns-discovery-onboarding.md` |
...
| Dedicated remote control ingress | Optional `[http.control]` binds a second mandatory-mTLS listener using the existing CA, registered-leaf fingerprint registry, and operator allow-list. Certificate identity is established before HTTP parsing. A separate pre-authentication cap contains TLS handshakes; authenticated operator/readiness work then moves to its own operator/doctor reserve. Only `/healthz`, `/readyz`, and `/operator/v1/*` survive route classification, every ingress phase has an absolute deadline, and ordinary-listener saturation cannot consume these ledgers. | `crates/oraclemcp-config/src/lib.rs`, `crates/oraclemcp-core/src/http/mod.rs`, `crates/oraclemcp-core/src/service_app.rs`, `crates/oraclemcp/src/main.rs` |
| HTTP request-rate limiting | Authenticated MCP POST/GET/DELETE and authorized `/operator/v1` requests pass through an asupersync `RateLimiter` registry after the server has a session/operator principal. Buckets are per traffic surface plus hashed server-derived principal, resident bucket count is bounded, and operator traffic has a separate bucket from MCP traffic so it is not starved by a hot MCP caller. Health/readiness observability routes are served before auth and are not charged to MCP request-rate buckets. Rejections return HTTP 429, `Retry-After`, and `AT_CAPACITY` with a redacted rate-limit snapshot. Raw bearer tokens, OAuth subjects, request ids, and caller-supplied identity strings are never limiter keys or response fields. Query-cost budgets remain deferred. | `crates/oraclemcp-core/src/http/mod.rs`, `oraclemcp-epic-060-f4xo.6.10`, `oraclemcp-040-epic-deferred-k6q.15` |
...
| OAuth metadata | `/.well-known/oauth-protected-resource` remains public when OAuth is enabled. | `crates/oraclemcp-core/src/http/mod.rs` |
| HTTP guards | `--listen` starts only with OAuth, mTLS client-certificate verification, or explicit `--allow-no-auth`; mTLS requests become application principals only when their leaf DER SHA-256 fingerprint is registered; remote bind requires explicit opt-in; Host and Origin guards protect loopback usage; missing auth returns WWW-Authenticate when OAuth is enabled. Native TLS/mTLS is served by the rustls listener when `[http.tls]` or `--tls-*` material is configured. A non-loopback native-TLS listener emits HSTS; loopback HTTPS omits it so localhost development is never browser-pinned. | `crates/oraclemcp/src/main.rs`, `crates/oraclemcp-auth/src/http_guard.rs`, `crates/oraclemcp-core/src/http/serve.rs` |
...
| Compatibility aliases | Legacy names such as `query`, `execute_approved`, `describe_table`, `get_ddl`, `get_object_source`, and others are still registered for client compatibility. Aliases inherit the canonical tools' safety defaults; in particular, omitted `commit` on `execute_approved` rolls DML back. | `README.md`, `crates/oraclemcp/src/registry.rs` |
...
| HTTP service instance guard | `serve --listen` acquires a private state-root `service-instance.json` lock after the TCP bind and before AppSpec startup. The root is `$XDG_STATE_HOME/oraclemcp`, or `$HOME/.local/state/oraclemcp` without XDG. A second HTTP service using that root fails closed with `ORACLEMCP_SERVICE_ALREADY_RUNNING` and reports the existing pid/listen/start metadata; different state roots can host independent listeners; `service status --json` exposes the same discovery block. | `crates/oraclemcp/src/main.rs`, `crates/oraclemcp/src/service_lifecycle.rs`, `docs/operations.md` |
...
- `Cargo.lock` resolves the published `oracledb = 0.9.0` and
  `oracledb-protocol = 0.9.0` crates from crates.io.
...
- `oraclemcp` consumes `oracledb = 0.9.0` from crates.io, declared in the
...
| IAM token | `use_iam_token` resolves a pre-fetched database token (a JWT) at connect time from one simple source: `token_file` (re-read every connect), `token_env` (else the built-in `ORACLEMCP_IAM_TOKEN`), or `token_exec` (argv array, no shell, timeout/output cap). It injects the token via the driver's `with_access_token` (sent as `AUTH_TOKEN`, TCPS-enforced). The token is a reference: resolved transiently, never persisted/rendered/logged; empty/missing is a typed fail-closed error; any token over a non-TCPS transport is refused before a `token_exec` command can spawn. `doctor` warns when the JWT `exp` is expired or within 5 minutes (diagnostic-only parse, no signature validation, token never printed). `iam_config_profile` still only parses (reserved for a future autonomous OCI-SDK token source). | Autonomous OCI-SDK token minting/refresh + live cloud coverage remain a real-cloud (C5) smoke item; do not claim autonomous IAM from the env/file/exec sources alone. |
