...
| HTTP stateful capacity | In served stateful mode, new lane allocation is admitted before the lane thread/factory can open a physical Oracle connection. N4b-finalized upper-bound defaults are 8 stateful lanes per principal bucket and 64 total host slots, with 1 operator and 1 doctor/readiness slot reserved outside regular agent admission; the defaults cite the CX-I6 measurement `tests/artifacts/perf/20260630-cx-i6-phase0-capacity/RESULTS.md`. Exhaustion returns `AT_CAPACITY` with `retry_after_ms`, HTTP 429 / `Retry-After`, and a redacted capacity snapshot. | `crates/oraclemcp-core/src/admission.rs`, `crates/oraclemcp-core/src/lane.rs`, `crates/oraclemcp-core/src/http.rs`, `crates/oraclemcp/src/main.rs` |
