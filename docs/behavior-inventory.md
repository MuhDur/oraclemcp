...
| Current release line | All `oraclemcp-*` package versions and `server.json` are aligned on the workspace release, while the independently-versioned thin driver/protocol pins are exact at 0.8.4. | `Cargo.toml`, crate `Cargo.toml` files, `server.json`, `Cargo.lock` |
...
- `Cargo.lock` resolves the published `oracledb = 0.8.4` and
  `oracledb-protocol = 0.8.4` crates from crates.io.
...
- `oraclemcp` consumes `oracledb = 0.8.4` from crates.io, declared in the
