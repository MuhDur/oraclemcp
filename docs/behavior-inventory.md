...
| Stdio resources | Initialize advertises served resources with `subscribe=false` and `listChanged=false`; `resources/list`, `resources/templates/list`, and `resources/read` are handled. Static reads return `oracle://capabilities` and `oracle://tools`; schema/object reads route through the same safe tool dispatch path as `oracle_schema_inspect`, `oracle_get_source`, and `oracle_get_ddl`. Prompts, completion, subscriptions, and lease-backed session resources are not advertised. | `crates/oraclemcp-core/src/server.rs`, `crates/oraclemcp-core/src/resources.rs`, `crates/oraclemcp-core/tests/mcp_conformance.rs` |
| HTTP endpoint | Streamable HTTP is mounted at `/mcp`; JSON response and stateful/session behavior are configurable. In stateful mode initialize issues `mcp-session-id`, subsequent POST/DELETE requests must present a known id, and DELETE invalidates it. | `crates/oraclemcp-core/src/http.rs` |
...
| Execute | Thin adapter reports rows affected, commit/rollback, savepoint rollback preview, and optional bounded DBMS_OUTPUT capture. | Cancellation-aware execute paths roll back dirty dispatcher work; preview DML always attempts rollback-to-savepoint and discards the lease if cleanup certainty is lost. |
...
| Context | Dispatch, DB, pool, serialization, and transport operations use explicit `&Cx` checkpoints around work that can block, loop, or cross I/O boundaries. | Keep new controlled APIs cancellation-aware instead of adding hidden blocking adapters. |
...
| Tests | `LabRuntime` and deterministic helpers cover quiescence, cancellation cleanup, loser drain, and preview-token races. | Preserve these tests as the evidence path for runtime changes. |
