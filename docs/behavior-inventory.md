...
| IAM token | `use_iam_token` resolves a pre-fetched database token (a JWT) at connect time from ONE of two SIMPLE sources — `token_file` (re-read every connect) or `token_env` (else the built-in `ORACLEMCP_IAM_TOKEN`) — and injects it via the driver's `with_access_token` (sent as `AUTH_TOKEN`, TCPS-enforced). The token is a reference: resolved transiently, never persisted/rendered/logged; empty/missing is a typed fail-closed error; any token over a non-TCPS transport is refused. `doctor` warns when the JWT `exp` is expired or within 5 minutes (diagnostic-only parse, no signature validation, token never printed). `iam_config_profile` still only parses (reserved for a future autonomous OCI-SDK token source). | Autonomous OCI-SDK token minting/refresh + live cloud coverage remain a real-cloud (C5) smoke item; do not claim autonomous IAM from the env/file sources alone. |
