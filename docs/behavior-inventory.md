...
| HTTP guards | `--listen` starts only with OAuth or explicit `--allow-no-auth`; remote bind requires explicit opt-in; Host and Origin guards protect loopback usage; missing auth returns WWW-Authenticate when OAuth is enabled. Native TLS/mTLS config is parsed but rejected by the v0.3.0 binary until the listener is wired. | `crates/oraclemcp/src/main.rs`, `crates/oraclemcp-auth/src/http_guard.rs`, `crates/oraclemcp-core/src/http.rs` |
