...
docker run -i --rm \
  ...
  ghcr.io/muhdur/oraclemcp:0.6.3
```
...
```sh
docker run -i --rm ghcr.io/muhdur/oraclemcp:0.6.3 info       # version, tools, transports
docker run -i --rm ghcr.io/muhdur/oraclemcp:0.6.3 --json doctor
```
...
Pin to an immutable tag (`:0.6.3`), not `:latest`, in any non-interactive
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: oraclemcp
          image: ghcr.io/muhdur/oraclemcp:0.6.3
          args:
            - serve
            - --listen
            - 0.0.0.0:7070
