...
docker run -i --rm \
  ...
  ghcr.io/muhdur/oraclemcp:0.7.1
```
...
```sh
docker run -i --rm ghcr.io/muhdur/oraclemcp:0.7.1 info       # version, tools, transports
docker run -i --rm ghcr.io/muhdur/oraclemcp:0.7.1 --json doctor
```
...
Pin to an immutable tag (`:0.7.1`), not `:latest`, in any non-interactive
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: oraclemcp
          image: ghcr.io/muhdur/oraclemcp:0.7.1
          args:
            - serve
            - --listen
            - 0.0.0.0:7070
