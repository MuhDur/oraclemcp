...
docker run -i --rm \
  ...
  ghcr.io/muhdur/oraclemcp:0.6.4
```
...
```sh
docker run -i --rm ghcr.io/muhdur/oraclemcp:0.6.4 info       # version, tools, transports
docker run -i --rm ghcr.io/muhdur/oraclemcp:0.6.4 --json doctor
```
...
Pin to an immutable tag (`:0.6.4`), not `:latest`, in any non-interactive
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: oraclemcp
          image: ghcr.io/muhdur/oraclemcp:0.6.4
          args:
            - serve
            - --listen
            - 0.0.0.0:7070
