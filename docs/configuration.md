...
| `profiles.require_security_feature_evidence` | bool | false | yes | no | 2 | Refuse reads when OLS/RAS/Data Redaction evidence is unreadable (default admits them with keyed security_feature_evidence: unavailable observations + audit record; a proven policy always refuses; protected profiles imply strict mode). |
